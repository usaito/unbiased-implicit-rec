...
class Model:
    ...
    def __init__(self, model_name: str):
        ...
        self.user_bias = np.load(f'../logs/{self.model_name}/embeds/user_bias.npy')
        self.item_bias = np.load(f'../logs/{self.model_name}/embeds/item_bias.npy')
...
class Evaluator:
    ...
    def evaluate(self, eps: float, pow: float, num_negatives: int = 100,
        ...
        for user in set(self.users):
            ...
            pos_items = self.items[indices]  # pos_items
            all_pos_items = self.pos_data[self.pos_data[:, 0] == user, 1]
            neg_items = np.random.permutation(
                np.setdiff1d(self.unique_items, all_pos_items))[:num_negatives]
