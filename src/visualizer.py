...
class Visualizer:
    ...
    def __init__(self, iters: int = 5) -> None:
        ...
        os.makedirs('../plots/results/ranking/', exist_ok=True)
        os.makedirs('../plots/curves/test/', exist_ok=True)

    def plot_rel_pred_results(self, eps: float = 5.0,
    ...
    def plot_ranking_results(self, eps: float = 5.0,
                             pow_list: List[float] = [0.5, 1.0, 2.0, 3.0, 4.0],) -> None:
        ...
        for pow in pow_list:
            for met in metrics:
                ret = pd.read_csv(
                    f'../logs/overall/results/ranking_{eps}_{pow}.csv',
    ...
    def plot_test_curves(self, eps: float = 5.0,
                         pow_list: List[float] = [0.5, 1.0, 2.0, 3.0, 4.0],) -> None:
        """Plot test curves."""
        for pow in pow_list:
            scatter_list = []
    ...
    def _load_and_save_results(
        ...
        for pow in pow_list:
            aoa_list = []
            for model in models:
                aoa_list.append(pd.read_csv(
                    f'../logs/{model}/results/ranking_{eps}_{pow}.csv', index_col=0))
            pd.concat([aoa_list[i] for i in np.arange(len(models))], 1).to_csv(
                f'../logs/overall/results/ranking_{eps}_{pow}.csv')
